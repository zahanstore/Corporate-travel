exports.handler = async function(event, context) {
  console.log('🚀 Airtable function - Proper date format');
  ...
  try {
    ...
    // Format date properly for Airtable (ISO format)
    let formattedDateTime = '';
    if (formData.date && formData.time) {
      // Combine date and time into ISO format
      const dateTimeString = `${formData.date}T${formData.time}:00.000Z`;
      formattedDateTime = new Date(dateTimeString).toISOString();
    }
    
    // Only use fields that are definitely writable
    const airtableData = {
      records: [
        {
          fields: {
            ...
            'Pickup Date/Time': formattedDateTime || 'Not provided',
            'Status': 'Inquired',
